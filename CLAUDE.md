...
├── jsonschema/v1/
├── scripts/
│   ├── export_jsonschema.py
│   └── check_dependencies.py   # 依存規律の番人 (§1.1)
└── tests/
...
`version.py` の `SCHEMA_VERSION` はデータ契約の SemVer。constants.py / schemas 各種
(manifest / platform / vehicle / calibration / trajectory 等) / topics 規約を
変えたら上げる。manifest は書き込み時の版を記録し、読み手は MAJOR 一致を要求する
(`DriveManifest.is_schema_compatible`)。
...
車両個体の物理パラメータ・CAN 仕様は platform ではなく vehicle.py が持つ (§4.8)。
Platform は車種を `vehicle_type` (VehicleType への参照 ID) で指すだけで、車両
パラメータをネストしない (車種の識別も `Platform.vehicle_type` が唯一の正)。
...
記録しないトピック (§5) は存在確認ループから外し (`recorded_only=True`)、無くても
Issue を出さない。一方 `allowed_keys` (想定外検知) には**含める**ので
`unexpected_topic_for_source` の誤検知にもならない。bag に入っていた場合のみ
`not_recorded_topic_present` (WARNING) を出す — データは使えるが記録設定ミスの
兆候なので、下流が選別できるよう想定外とは別 code にしている。ソース外かつ記録
しない組み合わせ (実車 bag の gt/object_attributes) は従来どおり
`unexpected_topic_for_source` (GT 混入の方が重大)。
...
### 4.8 vehicle.py: 車種と車両個体の分離 (フリート運用)
同一 platform に複数の車両個体がぶら下がる運用に対応するため、車両パラメータを
platform (旧 `VehicleParams`) から切り出した。`VehicleType` (車種の公称物理
パラメータ + CAN デフォルト `can_defaults`)、`Vehicle` (個体。所属 platform と
CAN 上書き `can_overrides` のみ。運用管理情報は studio 責務)、`CanSpec` (CAN 解釈
規約。`overlay` でフィールド単位合成) を新設。実効 CAN 仕様は
`Vehicle.effective_can_spec(vehicle_type)` が「override → default → 未定義」に畳む
(Vehicle は VehicleType を直接持たず platform 経由で解決)。CAN 語彙
`SpeedSignRule`/`BrakeNormalization` は唯一の消費者が `CanSpec` になったため
vehicle.py が定義元 (platform.py から移設。循環 import 回避)。キャリブ値は個体固有の
ため `CalibrationSet.vehicle` を必須化し、validation に
`validate_vehicle_consistency` (manifest×vehicle の platform/ID 一致) と
`calibration_vehicle_mismatch` (他車両キャリブ誤用 = ERROR) を追加。破壊的変更の
ため SCHEMA_VERSION を 0.2.0 へ (0.x では MINOR が破壊的変更を担う。jsonschema は
v1 のまま)。

### 4.9 manifest.tags: 階層ではなく属性で表現する
走行の分類軸には、正式フィールドにするほど普遍的ではないが文脈ごとに必要になる
ものがある (CARLA の `route_id`、実車の `campaign` 等)。これらを正式フィールドに
すると「他の source では意味を持たないフィールド」が増えていくため、source 固有の
概念は `tags: dict[str,str]` という**属性**で吸収する。studio の検索・フィルタは
タグベースで実装する想定。キーのみ命名規約 (小文字 snake_case) を課す。
`"Route_ID"`/`"route_id"` の表記揺れが混ざるとタグ検索が効かなくなるためで、
理由も実装も events.py の `source` (`SOURCE_PATTERN`) と同じ。ただし定数は
`manifest.py` にローカルに `TAG_KEY_PATTERN` として置く (events からの import は
「タグキーがイベント source の規約に従う」という誤読を招く)。値は自由文字列。

### 4.10 topics.py: QoS と記録対象を契約に含める
契約は「型」だけでなく publisher 側の QoS と、bag への記録対象かどうか
(`TopicContract.recorded`) も持つ。詳細は §5。`recorded` を `TopicRole` の値では
なく独立フラグにしたのは、role が「どのソースに存在するか」の軸で、「録るか」は
それと直交するため (`/tf` は実車・CARLA 両方に存在して両方で録らない)。
除外リスト定数にしなかったのは、契約の属性を契約の外側に置くことになるため。
...
- `gt/objects`: 全アクターの 3D BBox。map 座標。型は `vision_msgs/Detection3DArray`。
  `Detection3D.id`=instance_id (`"<route_index>_<carla_actor_id>"`。生存期間中安定・
  ルート内で一意 → nuScenes instance_token に対応づく)。results は実クラス仮説のみ
  (CARLA blueprint の type_id) で score は GT なので常に 1.0。visibility /
  attribute / num_lidar_pts 等は契約に含めず studio がオフライン算出 (§2.1)
- `gt/agent_plan`: Path (PDM-Lite 計画軌跡)
- `gt/depth_<pos>/image`: 32FC1 メートル深度。RGB と光学フレーム共有。**実車に無い**
- `gt/object_attributes`: std_msgs/String に JSON。**記録しない** (後述)
- `/clock`: sim 時刻

### QoS 規約
DDS は publisher と subscriber の QoS が両立しないと接続しないため、契約に含める。
core は ROS 非依存 (§1.1) なので文字列 enum で持ち、recorder のアダプタが
`rclpy.qos.QoSProfile` へ変換する。
- 既定 (`DEFAULT_QOS`): reliability=reliable, history=keep_last, depth=10,
  durability=volatile。センサ・GT・車両状態すべてこれ
- 例外は `/tf_static` のみ (`TRANSIENT_LOCAL_QOS`): depth=1,
  durability=transient_local (late joiner が後から最後の 1 通を受け取れる必要がある)

### 記録しないトピック (`recorded=False`)
publish はされるが bag に記録しないトピック。「契約として存在は認知するが記録対象
ではない」を `TopicContract.recorded` で表す。role (どのソースに存在するか) とは
直交する軸なので独立したフラグにしている。
- `/tf` (動的 tf): `gt/ego_odom` と情報が重複し、リプレイ時の tf 時刻補間問題を
  持ち込むため録らない (§2.6 / frames.py)。`/tf_static` は録る
- `gt/object_attributes`: CARLA ブリッジが将来の拡張余地として publish するが、
  visibility 等は studio の変換パイプラインがオフライン算出する方針なので録らない

validation はこれらを存在確認の対象から外す (`contracts_for_source(...,
recorded_only=True)`) ので、無くても Issue は出ない。逆に bag に入っていたら
設定ミスの兆候として WARNING (`not_recorded_topic_present`) を出す。
...
```
vehicle_type (車種。公称物理パラメータ + CAN デフォルト)
 └─ platform (sensor_rig + vehicle_type。同一構成で収録されたドライブ群)
     └─ vehicle (車両個体。CAN 上書き。1 platform に複数 = フリート運用)
         ├─ drive (1 走行 = nuScenes log と 1:1)
         └─ calibration (1 回 = 複数 calibrated_sensor に展開, 1:N。個体固有)
```
学習データセットの構成 (複数 platform を混ぜて学習に使うか) は studio の責務で
core は関与しない。

この階層に載らない分類軸 (CARLA の `route_id`、実車の収集キャンペーン
`campaign` 等) は、**階層を増やさず manifest の `tags` (自由 KV) で表現する**。
source 固有の概念を正式フィールドにすると他 source で意味を持たないフィールドが
増えるため (§4.9)。studio の検索・フィルタはタグベース。
